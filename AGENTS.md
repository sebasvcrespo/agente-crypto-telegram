...
- Lista fija `MULTI_PAIR_LIST`: XRP/BTC, ADA/BTC, ORDI/BTC, LINK/BTC, SUI/BTC, DOGE/BTC, SOL/BTC, PAXG/BTC, ETH/BTC, BNB/BTC, MSTRX/BTC (con sufijo `/BTC` para que `symbolForPair()` derive `XXX/BTC:BTC` → perpetuo Pionex `XXX_BTC_PERP`; NO pasar solo el ticker o fetchea `/USDT`).
...
## Motor multi-estrategia interno (`multiStrategyEngine.js` + `strategies.js` + `riskManager.js`)

- **Módulo independiente, 100% determinista (SIN IA).** No altera los flujos de 1H, multipar 30m (ambos **sugeridos/desactivados**) ni el multi-estrategia antiguo de `strategiesEngine.js` (sigue existiendo pero **no se ejecuta** desde el cron).
- **ÚNICO flujo automático activo.** Los análisis horario BTC 1H (`runHourlyAnalysis`) y escaneo multi-par 30m (`runMultiPairScan`) están **suspendidos** (crons comentados en `index.js`). El flujo 30m BTC (`runHalfHourlyAnalysis`) también está suspendido. El comando manual `Escaneo` sigue disponible.
- Cron `0 * * * *` con un `setTimeout` de **20s** (se ejecuta 20s después del cierre de la vela 1H, al inicio de cada hora). Respeta `Abierto/Cerrado`. Comando manual: `Estrategia` (fuerza, ignora Cerrado).
- Recorre los 12 pares (`INTERNAL_MULTI_STRATEGY_LIST`): **BTC/USDT** (el primero, par directo USDT) y los demás base BTC: XRP, ADA, ORDI, LINK, SUI, DOGE, SOL, PAXG, ETH, BNB, MSTRX (`/BTC`).
...
- `riskManager.calculateLevels`: **SL dinámico según la distancia entry→suggestedSlPrice** (proveniente de la estrategia ganadora). R = distancia SL; **TP1=1R, TP2=1.7R, TP3=2.5R** (33/33/34%). **Comisiones Pionex `FEE_TOTAL=1%` (0.5% apertura + 0.5% cierre) contempladas:** si la distancia TP1 (1R) no cubre el 1% → `calculateLevels` devuelve `null` y el trade se **invalida** (la IA nunca sugiere operar con TP1 neto ≤ 0). El dimensionamiento usa `idealNotional = riskBtc / (slDistancePct + FEE_TOTAL)` para que un SL completo (incl. fees) nunca supere el 10% de riesgo. **Riesgo porcentual por trade = 10%** del capital (`RISK_PERCENT`) → `riskBtc = CAPITAL_BTC × RISK_PERCENT`. **Capital BTC disponible** (`CAPITAL_BTC`, default 0.00010, configurable vía `/aumentocapital`) → `maxNotional = MAX_LEVERAGE × CAPITAL_BTC` (MAX_LEVERAGE = **10x**), `notional = min(ideal, max)`. Si el tope de 10x limita el tamaño, `riskCapped=true` y el riesgo real queda **por debajo** del 10% (seguridad ante todo). El leverage se valida además contra el max leverage real de Pionex por nocional.
...
- **Persistencia de estado:** `chatId`, `botStatus` y `capitalBtc` se guardan en `state.json` (no versionado, ver `.gitignore`) via `setChatId()`/`setBotStatus()`/`setCapitalBtc()`. Sobreviven a restarts/deploys para que el análisis no se salte por `chatId=null`. `setCAPITAL_BTC()` (de `riskManager.js`) se sincroniza al arrancar y al cambiar el capital con `/aumentocapital`.
- **Comando `/aumentocapital`:** actualiza el `capitalBtc` (BTC disponible para dimensionar posiciones, default 0.00010) en `state.json`. El motor multi-estrategia interna lo usa para `maxNotional = MAX_LEVERAGE × CAPITAL_BTC`. Está documentado en `/start` y `/help`.
- **Semántica de estado (invertida):** `Abierto` = alertas automáticas ACTIVAS (default al arrancar sin state.json); `Cerrado` = alertas PAUSADAS (el flujo automático activo — multi-estrategia interna — omite el ciclo). Los guards chequean `botStatus === "Cerrado"`. Nota: un `state.json` viejo con `"Cerrado"` (semántica anterior) dejará al bot pausado tras el deploy → escribir `Abierto` en Telegram una vez.
