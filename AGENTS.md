...
- `riskManager.calculateLevels`: **SL dinámico según la distancia entry→suggestedSlPrice** (proveniente de la estrategia ganadora). R = distancia SL; **TP1=1R, TP2=1.7R, TP3=2.5R** (33/33/34%). **Riesgo porcentual por trade = 10%** del capital (`RISK_PERCENT`) → `riskBtc = CAPITAL_BTC × RISK_PERCENT`. **Capital BTC disponible** (`CAPITAL_BTC`, default 0.00010, configurable vía `/aumentocapital`) → `idealNotional = riskBtc / slDistancePct`, `maxNotional = MAX_LEVERAGE × CAPITAL_BTC` (MAX_LEVERAGE = **10x**), `notional = min(ideal, max)`. Si el tope de 10x limita el tamaño, `riskCapped=true` y el riesgo real queda **por debajo** del 10% (seguridad ante todo). El leverage se valida además contra el max leverage real de Pionex por nocional.
