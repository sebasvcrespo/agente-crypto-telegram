...
- Cron `0,15,30,45 * * * *` con un `setTimeout` de **20s** (se ejecuta 20s después del cierre de la vela 15m, a los :00/:15/:30/:45). Respeta `Abierto/Cerrado`. Comando manual: `Estrategia` (fuerza, ignora Cerrado).
...
- `riskManager.calculateLevels`: **SL dinámico según volatilidad del par sobre ATR(15m)** — Baja (PAXG, BNB, ETH) **2×ATR**, Media (XRP, SOL, ADA, LINK) **2.5×ATR**, Alta (DOGE, SUI, ORDI) **3×ATR** (BTC cae en Media por default). R = distancia SL; **TP1=1R, TP2=1.7R, TP3=2.5R** (33/33/34%). **Riesgo porcentual por trade = 6.6%** del capital (`RISK_PERCENT`) → `riskBtc = CAPITAL_BTC × RISK_PERCENT`. **Capital BTC disponible** (`CAPITAL_BTC`, default 0.00010, configurable vía `/aumentocapital`) → `idealNotional = riskBtc / slDistancePct`, `maxNotional = MAX_LEVERAGE × CAPITAL_BTC` (MAX_LEVERAGE = **10x**), `notional = min(ideal, max)`. Si el tope de 10x limita el tamaño, `riskCapped=true` y el riesgo real queda **por debajo** del 6.6% (seguridad ante todo). El leverage se valida además contra el max leverage real de Pionex por nocional.
