...
- **Persistencia de estado:** `chatId`, `botStatus` y `capitalBtc` se guardan en `state.json` (no versionado, ver `.gitignore`) via `setChatId()`/`setBotStatus()`/`setCapitalBtc()`. Sobreviven a restarts/deploys para que el análisis horario no se salte por `chatId=null`. `setCAPITAL_BTC()` (de `riskManager.js`) se sincroniza al arrancar y al cambiar el capital con `/aumentocapital`.
